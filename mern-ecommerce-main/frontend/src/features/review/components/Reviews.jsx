...
export const Reviews = ({ productId, averageRating }) => {
    ...
    return (
        <Stack rowGap={3} alignSelf={"flex-start"} width={is480 ? "90vw" : is840 ? "25rem" : '40rem'}>
            <Stack>
                    ...
                    reviews?.length ? (
                        <Stack rowGap={3}>
                            <Stack rowGap={1} >
                                ...
                                <Typography variant='h6' color={'white'} >Based on {reviews.length} {reviews.length === 1 ? "Review" : "Reviews"}</Typography>
                            </Stack>
                            <Stack rowGap={2}>
                        ...
                        <Typography variant='h6' color={'white'} fontWeight={400}>{loggedInUser?.isAdmin ? "There are no reviews currently" : "Be the one to post review first"}</Typography>
                    )
                }
            </Stack>
            <Stack rowGap={2} sx={{ color: 'white' }}>
                ...
                writeReview ?
                    (
                        <Stack rowGap={3} position={'relative'} component={'form'} noValidate onSubmit={handleSubmit(handleAddReview)}>
                            ...
                            <Stack flexDirection={'row'} alignSelf={'flex-end'} alignItems={'center'} gap={5} columnGap={'.2rem'} sx={{ color: "white" }}>
                                <MotionConfig whileTap={{ scale: 1 }} whileHover={{ scale: 1.050 }}>
                                    <motion.div>
                                        <LoadingButton sx={{
                                            textTransform: "none", fontSize: is480 ? "" : "1rem", color: "white", '&:hover': {
                                                backgroundColor: 'white'
                                                , color: " #D4AF37"
                                            }
                                        }} size={is480 ? "small" : ""} loading={reviewStatus === 'pending'} type='submit' variant='contained'>Add review</LoadingButton>
                                    </motion.div>
                                    <motion.div>
                                        <Button onClick={() => setWriteReview(false)} size={is480 ? "small" : ""} variant='outlined' sx={{ textTransform: "none", fontSize: is480 ? "" : "1rem" }}>Cancel</Button>
                                    </motion.div>
                                </MotionConfig>
                            </Stack>
                        </Stack>
                    ...
                    !loggedInUser?.isAdmin ?
                        <motion.div onClick={() => setWriteReview(!writeReview)} whileHover={{ scale: 1.050 }} whileTap={{ scale: 1 }} style={{ width: "fit-content" }}>
                            <Button disableElevation size={is480 ? "medium" : 'large'} variant='contained' sx={{
                                color: theme.palette.primary.light, textTransform: "none", fontSize: "1rem", borderRadius: '6px', '&:hover': {
                                    backgroundColor: 'white'
                                    , color: " #D4AF37"
                                }
